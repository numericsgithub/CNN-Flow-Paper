...
use work.conv13_interleaving_Data_Aggregator_pkg.all;

entity conv13_interleaving is
    generic
    (
        XI_WIDTH        	: natural := 7;
...
end conv13_interleaving;

architecture Behavioral of conv13_interleaving is


    attribute KEEP_HIERARCHY : string;
    attribute KEEP_HIERARCHY of conv13_interleaving : entity is "yes";

	constant FIFO_START_THR     : natural := 2; -- todo well, maybe 4 is not always the best value here? But it is quite nice :D
    constant FIFO_SIZE          : natural := 466;
    ...
    signal data_aggregator_output          : conv13_interleaving_Data_Aggregator_output;
...
begin
    ...
    Data_Aggregator_for_FIFO: entity work.conv13_interleaving_Data_Aggregator
        port map
        (
            clk   => clk,
            reset => reset,
    ...
    begin
        if rising_edge(clk) then
            if fifo_data_counter >= FIFO_SIZE then
                assert false
                report "Interleaving layer conv13_interleaving: FIFO Overflow!"
                severity warning;
            end if;
        end if;
    end process;
