...
entity conv1_SlidingWindowController is
	port
	(
		clk  		: in std_logic;
		reset		: in std_logic;
...
end conv1_SlidingWindowController;

architecture structural of conv1_SlidingWindowController is


    attribute KEEP_HIERARCHY : string;
    attribute KEEP_HIERARCHY of conv1_SlidingWindowController : entity is "yes";
	...
	constant PADDING_TOP_OFFSET         : integer range 0 to KERNEL_SIZE := PADDING_SIZE - 0;
	constant PADDING_LEFT_OFFSET        : integer range 0 to KERNEL_SIZE := PADDING_SIZE - 0;
...
begin
    ...
    track_col_and_row_zero: process(row, column, padding_step_counter)
    begin
    ...
    begin
        if reset = '1' then
            ...
            if x_is_valid = '1' or (padding_step_counter >= FEATURE_MAP_SIZE * FEATURE_MAP_SIZE) then
                ...
                if True then -- After all configurations, we switch to the next pixel
	...
	pad_and_detect_y_valid_case: process(interleave_counter_signal,padding_step_counter, pad_column, column, row, x_is_valid)
    begin
        if x_is_valid = '1' or (padding_step_counter >= FEATURE_MAP_SIZE * FEATURE_MAP_SIZE) then
            ...
            if (0 >= CONFIGURATIONS - INTERLEAVINGS)
                ...
                and (((column-KERNEL_SIZE+2*PADDING_SIZE+PADDING_LEFT_OFFSET) mod STRIDE) = (STRIDE-1) and ((row-KERNEL_SIZE+PADDING_SIZE+PADDING_TOP_OFFSET) mod STRIDE) = (STRIDE-1)) then
                y_is_valid_signal <= '1';
            else
                y_is_valid_signal <= '0';
            end if;
