...
end conv2_interleaving;

architecture Behavioral of conv2_interleaving is


    attribute KEEP_HIERARCHY : string;
    attribute KEEP_HIERARCHY of conv2_interleaving : entity is "yes";


	constant FIFO_START_THR     : natural := 2;
    constant FIFO_LAST_READ_MAX : natural := INPUTS_PER_MUX - 1;
    constant FIFO_SIZE          : natural := 59;
    ...
    begin
        if rising_edge(clk) then
            if fifo_data_counter >= FIFO_SIZE then
                assert false
                report "Interleaving layer conv2_interleaving: FIFO Overflow!"
                severity warning;
            end if;
        end if;
    end process;
