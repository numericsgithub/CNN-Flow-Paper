...
end conv21_interleaving;

architecture Behavioral of conv21_interleaving is


    attribute KEEP_HIERARCHY : string;
    attribute KEEP_HIERARCHY of conv21_interleaving : entity is "yes";
    ...
    signal data_aggregator_output          : conv21_interleaving_Data_Aggregator_output;
...
begin
    ...
    Data_Aggregator_for_FIFO: entity work.conv21_interleaving_Data_Aggregator
        port map
        (
            clk   => clk,
            reset => reset,
    ...
    begin
        if rising_edge(clk) then
            if fifo_data_counter >= FIFO_SIZE then
                assert false
                report "Interleaving layer conv21_interleaving: FIFO Overflow!"
                severity warning;
            end if;
        end if;
    end process;
