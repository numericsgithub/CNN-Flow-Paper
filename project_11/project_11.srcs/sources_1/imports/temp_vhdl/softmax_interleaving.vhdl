...
use work.softmax_interleaving_Data_Aggregator_pkg.all;

entity softmax_interleaving is
    generic
    (
        XI_WIDTH        	: natural := 8;
		IN_INTERLEAVINGS    : natural := 1;
		INPUT_HOLD_CYCLES   : natural := 125;
...
end softmax_interleaving;

architecture Behavioral of softmax_interleaving is


    attribute KEEP_HIERARCHY : string;
    attribute KEEP_HIERARCHY of softmax_interleaving : entity is "yes";
    ...
    signal data_aggregator_output          : softmax_interleaving_Data_Aggregator_output;
...
begin
    ...
    fifo_MAIN_input_signal <=data_aggregator_output(0)  & data_aggregator_output(1)  & data_aggregator_output(2)  & data_aggregator_output(3) ;


    Data_Aggregator_for_FIFO: entity work.softmax_interleaving_Data_Aggregator
        port map
        (
            clk   => clk,
            reset => reset,
    ...
    fifo_1_output_signal <= fifo_MAIN_output_signal_pl(1)((XI_WIDTH*4)-1 downto (XI_WIDTH*3));

    fifo_2_output_signal <= fifo_MAIN_output_signal_pl(1)((XI_WIDTH*3)-1 downto (XI_WIDTH*2));

    fifo_3_output_signal <= fifo_MAIN_output_signal_pl(1)((XI_WIDTH*2)-1 downto (XI_WIDTH*1));

    fifo_4_output_signal <= fifo_MAIN_output_signal_pl(1)((XI_WIDTH*1)-1 downto (XI_WIDTH*0));
    ...
    begin
        if rising_edge(clk) then
            if fifo_data_counter >= FIFO_SIZE then
                assert false
                report "Interleaving layer softmax_interleaving: FIFO Overflow!"
                severity warning;
            end if;
        end if;
    end process;
