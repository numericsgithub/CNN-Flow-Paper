...
entity conv26_interleaving is
    generic
    (
        XI_WIDTH        	: natural := 7;
		CONFIGURATIONS      : natural := 1024;
		INTERLEAVINGS       : natural := 1;
		INPUTS_PER_MUX      : natural := 128
    );
    port
    (
        clk   : in std_logic;
...
end conv26_interleaving;

architecture Behavioral of conv26_interleaving is


    attribute KEEP_HIERARCHY : string;
    attribute KEEP_HIERARCHY of conv26_interleaving : entity is "yes";


	constant FIFO_START_THR     : natural := 2;
    constant FIFO_LAST_READ_MAX : natural := INPUTS_PER_MUX - 1;
    constant FIFO_SIZE          : natural := 9;
    ...
    begin
        if rising_edge(clk) then
            if fifo_data_counter >= FIFO_SIZE then
                assert false
                report "Interleaving layer conv26_interleaving: FIFO Overflow!"
                severity warning;
            end if;
        end if;
    end process;
