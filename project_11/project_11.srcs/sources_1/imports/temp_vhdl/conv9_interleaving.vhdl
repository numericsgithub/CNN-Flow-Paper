...
end conv9_interleaving;

architecture Behavioral of conv9_interleaving is


    attribute KEEP_HIERARCHY : string;
    attribute KEEP_HIERARCHY of conv9_interleaving : entity is "yes";

	constant FIFO_START_THR     : natural := 2; -- todo well, maybe 4 is not always the best value here? But it is quite nice :D
    constant FIFO_SIZE          : natural := 230;
    ...
    signal data_aggregator_output          : conv9_interleaving_Data_Aggregator_output;
...
begin
    ...
    Data_Aggregator_for_FIFO: entity work.conv9_interleaving_Data_Aggregator
        port map
        (
            clk   => clk,
            reset => reset,
    ...
    begin
        if rising_edge(clk) then
            if fifo_data_counter >= FIFO_SIZE then
                assert false
                report "Interleaving layer conv9_interleaving: FIFO Overflow!"
                severity warning;
            end if;
        end if;
    end process;
